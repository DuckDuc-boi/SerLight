...
client.on("ready", () => {
    console.log(`${client.user.username} đang online rồi đó!`);
    //database
    ...
    client.user.setPresence({
        status: "online",
        activity: {
            name: `${laysodep(client.guilds.cache.reduce((a, b) => a + b.memberCount, 0))} người dùng`,
            type: "WATCHING"
        }
    });
    if (process.env.TYPE_RUN == 'production') {
        ...
        setInterval(function() {
            client.user.setPresence({
                status: "online",
                activity: {
                    name: `Đang phục vụ ${laysodep(client.guilds.cache.reduce((a, b) => a + b.memberCount, 0))} người dùng`,
                    type: 'WATCHING'
                }
            });
...
client.on("guildCreate", async newguild => { //bot join server
    let embed = new MessageEmbed()
        .setTitle("Một server mới đã mời bot vào server của họ")
    ...
    client.channels.cache.get('735058241079214101').send(embed)
})

client.on("guildDelete", async oldguild => { //bot leave server
    let embed = new MessageEmbed()
        .setTitle("Bot đã bị đuổi khỏi server!")
...
client.on("message", async message => {
    ...
    if (!serverData){
        serverData = db.set(message.guild.id, { prefix: "sl.", logchannel: null, msgcount: true, defaulttts: null, botdangnoi: false, aiChannel: null, msgChannelOff: [], blacklist: false })
    }
    ...
    if (command) {
        ...
        if (blacklist_status === true && command.name !== 'blacklist') return message.channel.send('Server bạn đang nằm trong blacklist, vui lòng liên hệ owner của bot hoặc vào support server tại: https://top.gg/bot/');
